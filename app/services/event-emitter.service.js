...
setInterval(async () => {
    const copiedTransactions = transactionsData.map((transaction) => {
        return { ...transaction }
    })
    ...
    if (process.env.WORKER_POOL_ENABLED === '1') {
        ...
        try {
            const filteredTransactions =
                await workerPoolProxy.filterTransactions(
                    copiedTransactions,
                    ActiveConfiguration.getActiveConfigurationList()
                )
            if (Object.keys(filteredTransactions).length) {
                await new TransactionDao().bulkInsertTransctions(
                    filteredTransactions
                )
            }
        } catch (error) {
            console.error(
                `Something happened while filtering and bulk inserting data. Error: ${error}`
            )
        }
    } else {
        // TODO check
